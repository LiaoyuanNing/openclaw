...
- Feishu/Onboarding SecretRef guards: avoid direct `.trim()` calls on object-form `appId`/`appSecret` in onboarding credential checks, keep status semantics strict when an account explicitly sets empty `appId` (no fallback to top-level `appId`), and recognize env SecretRef `appId`/`appSecret` as configured so readiness is accurate. (#30903) Thanks @LiaoyuanNing.
