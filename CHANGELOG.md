...
- Feishu/Onboarding SecretRef guards: avoid direct `.trim()` calls on object-form `appId`/`appSecret` in onboarding credential checks, and keep status semantics strict when an account explicitly sets empty `appId` (no fallback to top-level `appId`) so readiness is not falsely reported. (#30903) Thanks @LiaoyuanNing.
